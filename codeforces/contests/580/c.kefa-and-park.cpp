...
void dfs(vector<vector<int>> &adj, vector<int> &cats, int currentNode,
         int parent, int catTolerance, int catsEncountered, int &ans) {
    ...
    if (neighbor != parent) {
      if (cats[neighbor - 1] == 1) {
        if (catsEncountered + 1 > catTolerance) {
          ...
          dfs(adj, cats, neighbor, currentNode, catTolerance,
              catsEncountered + 1, ans);
        }
      } else {

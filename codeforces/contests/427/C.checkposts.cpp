...
void dfs(vector<vector<int>> &adj, stack<int> &order, vector<bool> &visited,
         int currentNode) {
...
void dfsr(vector<vector<int>> &adjr, vector<bool> &visited,
          vector<int> &components, int currentNode) {
  visited[currentNode] = true;

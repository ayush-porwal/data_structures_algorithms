...
void dfs(vector<vector<int>> &adj, vector<int> &hatesAncestors,
         vector<bool> &hatedByAllChildren, int currentNode, int parent) {
  ...
  for (auto neighbor : adj[currentNode]) {
    if (neighbor != parent) {
      hatedByAllChildren[currentNode] =
          hatedByAllChildren[currentNode] && hatesAncestors[neighbor];
    }
  }

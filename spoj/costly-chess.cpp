...
int getPoint(int a, int b) { return (a * 8) + b; }

bool isInGrid(int a, int b) { return a >= 0 && a < 8 && b >= 0 && b < 8; }
  ...
  while (!pq.empty()) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (isInGrid(newA, newB)) {
        ...
        if (!visited[getPoint(newA, newB)] &&
            distance[u] + (currA * newA + currB * newB) < distance[v]) {

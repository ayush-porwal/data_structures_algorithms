...
void bfs(vector<string> &maze, vector<vector<bool>> &visited,
         pair<int, int> &start, vector<char> &path) {
  ...
  while (!nodes.empty() && !flag) {
    ...
    for (int i = 0; i < 4; i++) {
      pair<int, int> newStart = {currentNode.first + dx[i],
                                 currentNode.second + dy[i]};

      if (newStart.first < 0 || newStart.first >= maze.size() ||
          newStart.second < 0 || newStart.second >= maze[0].size()) {
        continue;
      }

      if (visited[newStart.first][newStart.second] ||
          maze[newStart.first][newStart.second] == '#') {
        continue;
      }
